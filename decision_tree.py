...
if __name__ == '__main__':
    ...
    # learn some baggings
    my_bagging_ensembles = dict()
    my_bagging_confusion_matrices = dict()
    my_bagging_accuracies = dict()
    my_bagging_precisions = dict()
    my_bagging_recalls = dict()
    
    sk_bagging_ensembles = dict()
    sk_bagging_confusion_matrices = dict()
    sk_bagging_accuracies = dict()
    sk_bagging_precisions = dict()
    sk_bagging_recalls = dict()
    
    for max_depth in [3, 5]:
        for bag_size in [5, 10]:
            #my bagging ensembles
            ensemble = bagging(Xtrn, ytrn, max_depth, bag_size)
            my_bagging_ensembles[(max_depth, bag_size)] = ensemble
            y_pred = [predict_example(x, ensemble) for x in Xtst] #transposed so I don't have too much clutter
            cm = confusion_matrix(ytst, y_pred)
            my_bagging_confusion_matrices[(max_depth, bag_size)] = cm
            my_bagging_accuracies[(max_depth, bag_size)] = metrics.accuracy_score(ytst, y_pred)
            my_bagging_precisions[(max_depth, bag_size)] = metrics.precision_score(ytst, y_pred)
            my_bagging_recalls[(max_depth, bag_size)] = metrics.recall_score(ytst, y_pred)
            
            #scikit-learns's bagging ensembles
            ensemble = BaggingClassifier(base_estimator = DecisionTreeClassifier(criterion="entropy", max_depth = max_depth), 
                                                  n_estimators = bag_size, 
                                                  max_samples = len(ytrn), 
                                                  max_features = max_depth)
            model = ensemble.fit(Xtrn, ytrn)
            sk_bagging_ensembles[(max_depth, bag_size)] = model
            y_pred = model.predict(Xtst)
            sk_bagging_confusion_matrices[(max_depth, bag_size)] = confusion_matrix(ytst, y_pred)
            sk_bagging_accuracies[(max_depth, bag_size)] = metrics.accuracy_score(ytst, y_pred)
            sk_bagging_precisions[(max_depth, bag_size)] = metrics.precision_score(ytst, y_pred)
            sk_bagging_recalls[(max_depth, bag_size)] = metrics.recall_score(ytst, y_pred)
                        
#    print(my_bagging_confusion_matrices)
#    print(sk_bagging_confusion_matrices)
    
#    input()
    my_boosting_ensembles = dict()
    my_boosting_confusion_matrices = dict()
    my_boosting_accuracies = dict()
    my_boosting_precisions = dict()
    my_boosting_recalls = dict()
    
    sk_boosting_ensembles = dict()
    sk_boosting_confusion_matrices = dict()
    sk_boosting_accuracies = dict()
    sk_boosting_precisions = dict()
    sk_boosting_recalls = dict()
    
    for max_depth in [1, 2]:
        for bag_size in [5, 10]:
            #my bagging ensembles
            ensemble = boosting(Xtrn, ytrn, max_depth, bag_size)
            my_boosting_ensembles[(max_depth, bag_size)] = ensemble
            y_pred = [predict_example(x, ensemble) for x in Xtst] #transposed so I don't have too much clutter
            my_boosting_confusion_matrices[(max_depth, bag_size)] = confusion_matrix(ytst, y_pred)
            my_boosting_accuracies[(max_depth, bag_size)] = metrics.accuracy_score(ytst, y_pred)
            my_boosting_precisions[(max_depth, bag_size)] = metrics.precision_score(ytst, y_pred)
            my_boosting_recalls[(max_depth, bag_size)] = metrics.recall_score(ytst, y_pred)
            
            #scikit-learns's bagging ensembles
            ensemble = AdaBoostClassifier(base_estimator = DecisionTreeClassifier(criterion="entropy", max_depth = max_depth), 
                                                  n_estimators = bag_size)
            model = ensemble.fit(Xtrn, ytrn)
            sk_boosting_ensembles[(max_depth, bag_size)] = model
            y_pred = model.predict(Xtst)
            sk_boosting_confusion_matrices[(max_depth, bag_size)] = confusion_matrix(ytst, y_pred)
            sk_boosting_accuracies[(max_depth, bag_size)] = metrics.accuracy_score(ytst, y_pred)
            sk_boosting_precisions[(max_depth, bag_size)] = metrics.precision_score(ytst, y_pred)
            sk_boosting_recalls[(max_depth, bag_size)] = metrics.recall_score(ytst, y_pred)
            
#    print(my_boosting_confusion_matrices)
#    print(sk_boosting_confusion_matrices)   